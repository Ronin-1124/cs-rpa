    ...
    if fallback is not None:
        ...
        if not _logged_mock_fallback:
            print(
                ...
                "Use python -m mock_dongdong demo for the supported local replica.",
                flush=True,
            )
...
def _prepare_live_edge(steal_focus: bool = True):
    from legacy.ui.windows import SW_SHOW, foreground, maximize, user32, win_from_hwnd
...
def _enum_child_hwnds(hwnd: int) -> list[tuple[int, str]]:
    from legacy.ui.windows import EnumWindowsProc, _text, user32
...
def _belongs_to_win(ctrl, win) -> bool:
    if ctrl is None:
        return False
    from legacy.ui.windows import user32
...
def click_inbox_tab(win, index: int = 0) -> bool:
    ...
    from legacy.ui.windows import click_abs
...
def _fill_live_edge(
    ...
    from legacy.ui.windows import click_abs, foreground, maximize, paste_text, user32, win_from_hwnd
    from legacy.ui.windows import HWND_NOTOPMOST, SWP_NOMOVE, SWP_NOSIZE, SWP_SHOWWINDOW
...
def _dump_landmarks(win) -> None:
    from legacy.ui.windows import _text, user32
