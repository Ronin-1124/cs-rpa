...
if __name__ == "__main__" and ("--mock" in sys.argv or os.environ.get("CS_RPA_MOCK") == "1"):
    raise SystemExit("The old UIA mock runner is retired. Run: python offline_demo.py")
...
from adapters.qianniu import make_adapter as make_qianniu  # noqa: E402
...
def mock_health_ok() -> bool:
    import urllib.request

    try:
        urllib.request.urlopen("http://127.0.0.1:18765/api/health", timeout=0.8)
        return True
    except Exception:
        return False


def ensure_mock_workbench() -> None:
    """Start the fake page server if needed, then open Edge 本地测试."""
    if not mock_health_ok():
        log({"event": "starting", "platform": "jingmai", "note": "mock server down; starting run-mock.cmd"})
        subprocess.Popen(
            ["cmd.exe", "/k", str(ROOT / "run-mock.cmd")],
            cwd=str(ROOT),
        ...
        while time.time() < deadline:
            ...
            time.sleep(0.4)
        if not mock_health_ok():
            log({"event": "error", "platform": "jingmai", "error": "mock server did not start on 127.0.0.1:18765"})
            return
        log({"event": "ready", "platform": "jingmai", "note": "mock server up"})
    from ui.web_jingmai import _find_live_dongdong_edge

    deadline = time.time() + 8
    while time.time() < deadline:
        if _find_live_dongdong_edge() is not None:
            return
        time.sleep(0.5)
    open_dongdong()


def show_qianniu() -> bool:
    chat = find_window("AliWorkbench", "接待中心", visible_only=False, min_size=0)
    if chat is None:
        log({"event": "error", "platform": "qianniu", "error": "接待中心 window not found"})
        return False
    user32.ShowWindow(chat.hwnd, SW_SHOW)
    user32.ShowWindow(chat.hwnd, SW_MAXIMIZE)
    foreground(chat, restore=False)
    log({"event": "ready", "platform": "qianniu", "hwnd": chat.hwnd, "title": chat.title})
    return True


def open_dongdong() -> None:
    from ui.web_jingmai import (
...
def ocr_qianniu_notify(n) -> tuple[str, str, Path | None]:
    from PIL import ImageGrab
    from ui.ocr import ocr_lines, parse_qianniu_notify_lines
...
def ocr_qianniu_chat() -> tuple[str, str]:
    from PIL import ImageGrab
    from ui.ocr import compact_ocr, ocr_lines, parse_qianniu_notify_lines
...
def ask_and_fill_qianniu(cfg: dict, client: OpenClawClient, buyer: str, text: str) -> None:
    from ui.ocr import looks_like_customer_text
...
class QianniuWalk:
    ...
    def arm(self) -> None:
        from ui.ocr import find_daihui_click, parse_qianniu_session_rows
...
def handle_qianniu(cfg: dict, client: OpenClawClient, adapter, seen: set) -> None:
    # Toast clicks fight the ordered walk. Queue walk owns 接待中心.
    return
...
def main() -> int:
    ...
    parser.add_argument(
        "--mock",
        action="store_true",
        help="use 咚咚工作站·本地测试 only; never touch the real shop window",
    )
    parser.add_argument("--offline", action="store_true", help="local mock only: use fixed demo replies without OpenClaw")
    args = parser.parse_args()
    if args.offline and (not args.mock or args.platform != "jingmai"):
        parser.error("--offline requires --mock --platform jingmai")
    if args.mock:
        os.environ["CS_RPA_MOCK"] = "1"
    if args.restore:
        from ui.web_jingmai import restore_live_inbox
    ...
    cfg = load_config()
    if args.offline:
        from offline_replies import OfflineReplyClient

        client = OfflineReplyClient()
        cfg["auto_send"] = True
        log({"event": "reply_provider", "provider": "offline_templates", "mock": True})
    else:
    ...
    seen: set = set()
    qn_walk = QianniuWalk()
    jm_walk = JingmaiWalk(pending_only=args.offline)
    # Snapshot already-visible popups so we do not click/steal focus on startup.
    n = find_notify(title_equals="消息提醒")
    if n and n.visible:
        seen.add(("qn-notify", n.hwnd, n.left, n.top, n.width, n.height))
    from ui.web_jingmai import _find_live_dongdong_edge, dongdong_missing_hint, mock_mode

    if "jingmai" in platforms and mock_mode():
        ensure_mock_workbench()
    ...
    print(
        ...
        f"auto_send={bool(cfg.get('auto_send'))}; if a person clicks 咚咚, run run-restore-jm.cmd"
