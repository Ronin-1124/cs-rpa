...
知识 ID 校验不能证明回复中的每句话都被资料支持，所以默认采用审核模式；没有实时库存、订单或价格查询组件时，不把历史 CSV 视为实时系统。

会话可由同事接管。模型返回时再次检查会话最新消息和接管状态，过期结果不能恢复自动接待或覆盖新消息。等待期间仍同步新消息，同事结果恢复时使用最新历史。

## 发送与通知

发送队列以会话和输入消息 ID 去重。审核通过只进入 `ready` 状态，实际发送前重新打开目标客户、检查 DOM 最新消息、确认编辑框没有其他草稿，并原子检查暂停/接管/取消状态。点击后只有发现新的客服消息 ID 且正文一致才记录 `sent`；允许 Chromium contenteditable 产生的连续空行差异，保留实际网页文本和消息 ID。
