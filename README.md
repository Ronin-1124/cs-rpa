...
3. 在“接待设置”独立选择页面来源（模拟页面或真实京麦）和回复方式（填写草稿或自动发送）。两种页面都支持两种回复方式。
4. 点击“开始接待”，再从模拟客户控制台创建客户并发送问题。程序通过工作台 DOM 读取消息，回复出现在管理页面。
5. “填写草稿”由 RPA 将回复填入客服网页输入框，不点击发送，管理页保留回复副本。需要发送时可在网页手动发送，或在管理页修改回复后点击“发送回复”。“自动发送”由 RPA 填入、点击发送并确认结果。输入框已有其他手写内容时不会覆盖；本次接待中程序自己填入的草稿可更新。
...
`jingmai` 接入使用独立浏览器配置目录打开 `https://dongdong.jd.com/`，由使用者完成登录。页面来源不限制回复方式：填写草稿不点击发送，自动发送使用 DOM 填入和发送确认链路。真实适配的填入与发送链路已通过拦截网络的合成页面回归，实际京麦发送效果仍需现场验证。
...
自动发送模式将有效回复加入发送队列；填写草稿模式保留草稿并尝试填入网页，同一版本不会重复填入。网页填入受阻时管理页显示原因，完全停止后重新开始接待可重试。网页账号若处于挂起状态，需在京东页面自行切换为接待状态才能接收新咨询。
